...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      title: "LLM 偏好",
      description:
        "AthenaLLM 可以與多家 LLM 提供商合作。這將是處理聊天的服務。",
    },
    userSetup: {
      title: "使用者設定",
    ...
    survey: {
      title: "歡迎使用 AthenaLLM",
      description: "幫助我們為您的需求打造 AthenaLLM。可選。",
      email: "您的電子郵件是什麼？",
      useCase: "您將如何使用 AthenaLLM？",
      useCaseWork: "用於工作",
      useCasePersonal: "用於個人使用",
      useCaseOther: "其他",
      comment: "您是如何聽說 AthenaLLM 的？",
      commentPlaceholder:
    ...
    workspace: {
      title: "創建您的第一個工作區",
      description: "創建您的第一個工作區並開始使用 AthenaLLM。",
    },
  },
  common: {
  ...
  welcomeMessage: {
    part1:
      "歡迎使用 AthenaLLM，AthenaLLM 是由 Mintplex Labs 開發的開源 AI 工具，它能將任何內容轉換成可供查詢和對話的訓練模型對話機器人。AthenaLLM 採用 BYOK（自備金鑰）軟體模式，除了您想使用的服務之外，本軟體不收取任何訂閱費、費用或其他費用。",
    part2:
      "AthenaLLM 是將 OpenAI、GPT-4、LangChain、PineconeDB、ChromaDB 和其他強大 AI 產品整合在一起的最簡單方法，它能透過簡潔的套件，輕鬆地將您的生產力提高 100 倍。",
    part3:
      "AthenaLLM 可以完全在您的本機電腦上執行，而且使用極少的資源，您甚至不會注意到它的存在！不需要 GPU。同時也支援雲端和企業內部部署。\nAI 工具生態系統日新月異，AthenaLLM 讓使用變得更加簡單。",
  ...
  api: {
    title: "API 金鑰",
    description:
      "API 金鑰允許持有者以程式化方式存取和管理此 AthenaLLM 系統。",
    link: "閱讀 API 文件",
  ...
  llm: {
    title: "LLM 偏好設定",
    description:
      "這些是您偏好的 LLM 對話與嵌入提供者的憑證和設定。確保這些金鑰是最新且正確的，否則 AthenaLLM 將無法正常運作。",
    provider: "LLM 提供者",
  },
  transcription: {
    title: "語音轉錄模型偏好設定",
    ...
    provider: "語音轉錄提供者",
    "warn-start":
      "在記憶體或處理器資源有限的電腦上使用本機 Whisper 模型，處理媒體檔案時可能會造成 AthenaLLM 停頓。",
  ...
  embedding: {
    ...
    "desc-end":
      "嵌入是將文字轉換成向量的過程。這些憑證是用於將您的檔案和提示詞轉換成 AthenaLLM 可以處理的格式。",
    provider: {
      title: "向量嵌入提供者",
      description: "使用 AthenaLLM 的原生嵌入引擎時，不需要任何設定。",
    },
  },
  text: {
    title: "文字分割與區塊化偏好設定",
  ...
  vector: {
    title: "向量資料庫",
    description:
      "這些是您的 AthenaLLM 系統運作方式的憑證和設定。確保這些金鑰是最新且正確的，這點非常重要。",
    provider: {
      title: "向量資料庫提供者",
  ...
  privacy: {
    title: "隱私與資料處理",
    description:
      "這是您針對已連線的第三方供應商和 AthenaLLM 如何處理您的資料的設定。",
    llm: "LLM 選擇",
    embedding: "向量嵌入偏好設定",
    vector: "向量資料庫",
    anonymous: "已啟用匿名統計資訊",
  ...
  "main-page": {
    ...
    checklist: {
      title: "開始使用",
      tasksLeft: "個任務未完成",
      completed: "你已經走在成為AthenaLLM專家的路上！",
      dismiss: "關閉",
      tasks: {
        create_workspace: {
          title: "建立工作空間",
