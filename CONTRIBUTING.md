# Contributing to AthenaLLM

AthenaLLM is an open-source project and we welcome contributions from the community.
...
1. If you are modifying the permission system for a new role or something custom, you are likely better off forking the project and building your own version since this is a core part of the project and is only to be maintained by the AthenaLLM team.
...
The core library is written in Node.js. There are additional sub-repositories for the embed widget and browser extension. These are not part of the core AthenaLLM project, but are maintained by the AthenaLLM team.
...
Changes to the core AthenaLLM project are released through the `master` branch. When a PR is merged into `master`, a new version of the package is published to Docker and GitHub Container Registry under the `latest` tag.
...
Changes to the desktop app are downstream of the core AthenaLLM project. Releases of the desktop app are published at the same time as the core AthenaLLM project. Code from the core AthenaLLM project is copied into the desktop app into an Electron wrapper. The Electron wrapper that wraps around the core AthenaLLM project is **not** part of the core AthenaLLM project, but is maintained by the AthenaLLM team.

## License

By contributing to AthenaLLM (this repository), you agree to license your contributions under the MIT license.